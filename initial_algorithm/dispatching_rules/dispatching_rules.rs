...
fn flow_hint_from_hyper(flow: &Option<String>) -> Result<Option<FlowHint>> {
    ...
    Ok(match flow.to_lowercase().as_str() {
        // Current track names
        "flow_shop" | "hybrid_flow_shop" => Some(FlowHint::Strict),
        "job_shop" => Some(FlowHint::Parallel),
        "fjsp_medium" | "fjsp_high" => None,
        // Legacy names (kept for backwards compatibility)
        "strict" => Some(FlowHint::Strict),
        "parallel" => Some(FlowHint::Parallel),
        "random" | "complex" | "chaotic" => None,
        other => return Err(anyhow!("Unknown flow type: {}", other)),
    })
}
