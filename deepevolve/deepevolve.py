...
# Transient API errors that should be retried automatically
_TRANSIENT_API_ERRORS = (
    "BadGateway", "ServiceUnavailable", "InternalServerError",
    "APIConnectionError", "Timeout", "RateLimitError",
    "502", "503", "529",
)

async def _api_call_with_retry(coro_fn, max_retries: int = 5, base_delay: float = 10.0):
    """Call an async coroutine function, retrying on transient API errors."""
    for attempt in range(max_retries + 1):
        try:
            return await coro_fn()
        except Exception as e:
            err = str(e)
            is_transient = any(t in err for t in _TRANSIENT_API_ERRORS)
            if is_transient and attempt < max_retries:
                delay = base_delay * (2 ** attempt)
                logging.getLogger(__name__).warning(
                    f"Transient API error (attempt {attempt+1}/{max_retries}): {err[:120]}. "
...
class DeepEvolve:
    ...
    async def run(
        ...
        for i in range(start_iteration, max_iterations):
            ...
            research_plans, search_results, research_reports = (
                await _api_call_with_retry(lambda: self.researcher.run(
                    parent,
                    inspirations,
            ...
            all_diff_text, all_program_code = await _api_call_with_retry(lambda: self.coder.run(
                new_idea,
                parent,
                inspirations,
                trace_id=self.trace_id,
    ...
    def _log_iteration(
        ...
        self._print_iteration_summary(iteration, parent, child, elapsed_time)


    def _print_iteration_summary(self, iteration, parent, child, elapsed_time):
        m  = child.metrics
        pm = parent.metrics

        # Build track list dynamically from metrics keys
        # Supports neuralnet (quality_nN), job-scheduling (quality_*), or any other format
        tracks = []
        if any(k.startswith("quality_n") for k in m):
            # Neuralnet format: quality_n4, quality_n7, etc.
            for key in sorted(k for k in m if k.startswith("quality_n")):
                n = key[len("quality_n"):]
                tracks.append((f"n_hidden={n}", key, "time_s", f"delta_pct_n{n}"))
        else:
            # Job-scheduling fallback
            tracks = [
                ("FlowShop",       "quality_flow_shop",        "time_flow_shop",        "delta_pct_flow_shop"),
                ("HybridFlowShop", "quality_hybrid_flow_shop", "time_hybrid_flow_shop", "delta_pct_hybrid_flow_shop"),
                ("JobShop",        "quality_job_shop",         "time_job_shop",         "delta_pct_job_shop"),
                ("FjspMedium",     "quality_fjsp_medium",      "time_fjsp_medium",      "delta_pct_fjsp_medium"),
                ("FjspHigh",       "quality_fjsp_high",        "time_fjsp_high",        "delta_pct_fjsp_high"),
            ]

        is_best    = self.database.best_program_id == child.id
        combined   = m.get("combined_score",  m.get("avg_quality", 0))

        # Compare against the overall best, not the parent
        # This ensures delta always shows improvement vs current best
        best_prog  = self.database.get_best_program(metric="combined_score")
        if best_prog and best_prog.id != child.id:
            best_combined = best_prog.metrics.get("combined_score", best_prog.metrics.get("avg_quality", 0))
        else:
            # Child IS the new best - compare against previous best (parent chain)
            best_combined = pm.get("combined_score", pm.get("avg_quality", 0))
        delta    = combined - best_combined
        short_id = child.id[:8]

        if is_best:
            header = f"[bold green]* NEW BEST   iter {iteration+1}  {short_id}  ({delta:+.1f} -> {combined:.1f})[/bold green]"
        elif delta > 0:
            header = f"[bold yellow]  IMPROVED   iter {iteration+1}  {short_id}  ({delta:+.1f} -> {combined:.1f})[/bold yellow]"
        else:
            header = f"[dim]  no change   iter {iteration+1}  {short_id}  ({delta:+.1f} -> {combined:.1f})[/dim]"

        W = 68
        self.console.print()
        self.console.print("[bold]" + chr(9473) * W + "[/bold]")
        self.console.print(" " + header)
        self.console.print("[bold]" + chr(9472) * W + "[/bold]")
        self.console.print(f"  {'Track':<20} {'Quality':>10}  {'Delta%':>8}  {'Time':>8}  {'Change':>10}")
        self.console.print(f"  {'-'*20} {'-'*10}  {'-'*8}  {'-'*8}  {'-'*10}")

        for label, qk, tk, dk in tracks:
            q   = m.get(qk,  0)
            t   = m.get(tk,  0)
            d   = m.get(dk,  0)
            pq  = pm.get(qk, 0)
            ...
            d_str   = f"{d:+.2f}%" if d != 0 else " 0.00%"
            self.console.print(
                f"  {label:<20} {q:>10,.0f}  {d_str:>8}  {t:>7.1f}s  {chg_str}"
            )

        total_t = m.get("total_time_s", 0)
        avg_q   = m.get("avg_quality", combined)
        self.console.print("[bold]" + chr(9472) * W + "[/bold]")
        self.console.print(
            f"  {'avg_quality':<20} {avg_q:>10,.1f}  {'':>8}  {total_t:>7.1f}s"
            f"  (wall: {elapsed_time:.0f}s)"
        )
        self.console.print("[bold]" + chr(9473) * W + "[/bold]")
        self.console.print()
