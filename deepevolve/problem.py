    ...
    async def evaluate(
        ...
        while retry_count <= self.max_retry_times:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # We evict them unconditionally so the fresh files in tmpdir are
                # always imported. We do NOT restore them afterwards - restoring
                # a stale cached module object would cause every iteration after
                # the first to re-execute the previous iteration's RUST_CODE.
                # rust_code.py must be evicted before deepevolve_interface.py so
                # that the `from rust_code import RUST_CODE` in the interface
                # picks up the freshly written file, not a cached module.
                ...
                try:
                    ...
                    # Remove tmpdir from sys.path. Do NOT restore evicted modules -
                    # keeping them evicted ensures the next iteration always imports
                    # fresh code rather than a stale cached version.
